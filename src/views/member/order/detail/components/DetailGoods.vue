    ...
    <div class="order-info">
        ...
        <table class="goods-table">
            ...
            <tbody>
                <tr v-for="sku in order.skus" :key="sku.id">
                    ...
                    <td>¥{{ sku.curPrice.toFixed(2) }}</td>
                    <td>{{ sku.quantity }}</td>
                    <td>¥{{ sku.realPay.toFixed(2) }}</td>
                    <td>¥{{ sku.realPay.toFixed(2) }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="info-box">
            ...
            <dl>
                ...
                <dd>
                    {{
                        order.receiverMobile.substring(0, 3) +
                        '****' +
                        order.receiverMobile.substr(7)
                    }}
                </dd>
            </dl>
            <dl>
        ...
        <div class="info-box">
            <dl>
                <dt>支付方式：</dt>
                <dd v-if="order.payType === 1">在线支付</dd>
                <dd v-if="order.payType === 2">货到付款</dd>
            </dl>
            <dl>
                <dt>送货时间：</dt>
                <dd v-if="order.deliveryTimeType === 1">
                    不限(适合于家庭地址)
                </dd>
                <dd v-if="order.deliveryTimeType === 2">
                    工作日(适合于家庭地址)
                </dd>
                <dd v-if="order.deliveryTimeType === 3">
                    双休日、假日送货(适合于家庭地址)
                </dd>
            </dl>
        </div>
